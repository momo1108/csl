	...
	public static void move(int[] player, int[][] room) {
		...
		for(int i=0; i<player[2]; i++) {
			...
			switch(direction) {
				case 1:
					for(; j<=move; j++) {
						if(player[1]+j>Y || room[player[0]][player[1]+j]==0) return;
					}
					j--;
					player[1] += j;
					break;
				case 2:
					for(; j<=move; j++) {
						if(player[0]+j>X || room[player[0]+j][player[1]]==0) return;
					}
					j--;
					player[0] += j;
					break;
				case 3:
					for(; j<=move; j++) {
						if(player[1]-j<1 || room[player[0]][player[1]-j]==0) return;
					}
					j--;
					player[1] -= j;
					break;
				case 4:
					for(; j<=move; j++) {
						if(player[0]-j<1 || room[player[0]-j][player[1]]==0) return;
					}
					j--;
					player[0] -= j;
					break;
